...
public class RPnHttpPoller implements RPnResetableListener {
    ...
    public void startsListening() {
		...
		/*
  		 * we will assume just NOTEBOARD hits for now..
		 */
    ...
    public void parseMessage(BufferedInputStream inputStream) {

	URLConnection rpnMediatorConn = rpnMediatorURL.openConnection();

	try {

		ObjectInputStream objStreamer = new ObjectInputStream(inputStream);

		String header = objStreamer.readUTF();
		if (header.compareTo("TXT") == 0) {

			String text;
			StringBuffer fullText = new StringBuffer();
			Boolean buffFlag = false;
			while ((text = objStreamer.readUTF()) != null) {
				buffFlag = true;
				fullText.append(text);
			}

			parseMessageText(fullText.toString());

		} else if (header.compareTo("OBJ") == 0) {
			parseMessageObject(objStreamer.readObject());
